...
class InvoiceController extends Controller
{
        ...
        try {
            ...
            if ($action === 'create_send') {
                return $this->sendInvoiceEmail($invoice)
                    ? redirect()->route('invoices.show', $invoice)->with('success', 'Facture creee et envoyee par email.')
                    : redirect()->route('invoices.show', $invoice)->withErrors(['email' => 'Facture creee, mais l envoi email a echoue. Verifiez l email du client et le SMTP.']);
            }
    ...
    public function email(Facture $invoice): RedirectResponse
    {
        ...
        try {
            ...
            return back()->with('success', 'Facture envoyee par email.');
        } catch (Throwable $exception) {
            return back()->withErrors(['email' => 'Impossible d envoyer la facture par email. Verifiez la configuration SMTP et l adresse du client.']);
        }
    }
    ...
    public function destroy(Facture $invoice): RedirectResponse
    {
        ...
        DB::transaction(function () use ($invoice): void {
            foreach ($invoice->details as $detail) {
                if ($detail->produit) {
                    // La suppression remet le stock en place pour garder une coherence totale apres annulation.
                    $this->stockService->move(
                        $detail->produit,
                        auth()->user(),
                        'entree',
                        (int) $detail->quantity,
                        "Retour stock apres suppression de la facture {$invoice->invoice_number}"
                    );
                }
            }
    ...
    // Cette methode envoie la facture au client et renvoie vrai si tout s est bien passe.
    private function sendInvoiceEmail(Facture $invoice): bool
    {
        if (! $invoice->client?->email) {
            return false;
        }
        ...
        $invoice->load(['client', 'user', 'details.produit']);
        $settings = $this->parameterService->current();
        $this->parameterService->applyMailConfiguration();
