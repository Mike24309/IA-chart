...
class AiAuditService
{
    ...
    public function analyze(User $user, bool $force = false): array
    {
        $settings = $this->aiSettingsService->current();
        $metrics = $this->buildMetricsSnapshot($settings);
    ...
    public function analyzeLocal(User $user): array
    {
        $settings = $this->aiSettingsService->current();
        $metrics = $this->buildMetricsSnapshot($settings);
    ...
    public function answerQuestion(User $user, string $question): array
    {
        ...
        $directAnswer = $this->buildDirectDataAnswer($question, $liveMetrics);
    ...
    public function buildReportContext(User $user): array
    {
        $verifiedAnalysis = $this->buildVerifiedAnalysis($user);
    ...
    private function buildVerifiedAnalysis(User $user): array
    {
        $settings = $this->aiSettingsService->current();
        $metrics = $this->buildMetricsSnapshot($settings);
        $signature = hash('sha256', json_encode($metrics));
    ...
    private function buildMetricsSnapshot($settings): array
    {
        $now = now();
...
Le JSON doit avoir cette structure :
{
  ...
  "download_summary": {
    "title": "titre de rapport pour la direction",
    "summary": "rapport ecrit en 5 a 8 phrases, comme un compte rendu au patron",
    "conclusion": "decision conseillee en 2 a 4 phrases"
  }
}
...
Tu peux repondre a n importe quel message normal de l utilisateur.
...
Si la question concerne les ventes, le stock, les factures, les utilisateurs ou les alertes, utilise l analyse si elle existe.
...
Si aucune analyse n existe, reponds quand meme naturellement et propose de lancer l analyse pour une reponse plus precise.
...
PROMPT;
    ...
    private function callGemini(string $prompt, string $model, ?array $fallbackMetrics = null, ?string $fallbackQuestion = null, bool $allowFallback = true): array
    {
            ...
            foreach ($candidateModels as $candidateModel) {
                try {
                    $response = Http::timeout(40)
                        ->connectTimeout(15)
                        ->retry(4, 1800, function ($exception) {
    ...
    private function extractStructuredJsonWithRepair(array $responsePayload, string $model, string $schemaType): array
    {
        $content = data_get($responsePayload, 'choices.0.message.content');
        ...
        $repairPayload = $this->callGemini($repairPrompt, $model, null, null, false);
    ...
    private function buildFallbackAnalysis(array $metrics): array
    {
        ...
        return [
            ...
            'global_score' => [
                'score' => $score,
                'explanation' => 'Note calculee localement car la reponse Gemini n etait pas encore lisible en JSON.',
                'breakdown' => [
    ...
    private function buildDirectDataAnswer(string $question, array $liveMetrics): ?string
    {
        ...
        if (
            ...
            return "Vous avez actuellement {$productsInStockCount} produit(s) avec du stock disponible, sur {$totalProducts} produit(s) enregistres, pour un total de {$totalUnits} unite(s). Parmi eux, {$criticalProducts} sont critiques, {$watchProducts} sont a surveiller, {$healthyProducts} sont stables et {$dormantProducts} sont dormants.";
        }
        ...
        if (
            ...
            return "Le stock contient actuellement {$productsInStockCount} produit(s) avec du stock disponible sur {$totalProducts} produit(s) enregistres, pour {$totalUnits} unite(s). On compte {$criticalProducts} produit(s) critiques, {$watchProducts} a surveiller, {$healthyProducts} stables et {$dormantProducts} dormants.";
        }
